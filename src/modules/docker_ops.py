...
class DockerConfig(BaseModel):
    """Docker配置模型"""
    socket_path: str = Field(default_factory=lambda: get_config('docker.socket_path', 'unix://var/run/docker.sock'))
...
class DockerOpsTool(BaseTool):
    ...
    def _list_containers(self, client) -> str:
        ...
        if client:
            ...
            # Fallback到命令行
            return self._run_docker_command("docker ps -a --format 'table {{.ID}}\\t{{.Names}}\\t{{.Status}}\\t{{.Image}}'")
    ...
    def _get_container_status(self, client, container_name: str) -> str:
        ...
        if client:
            try:
                ...
                uptime = format_duration(int(info['State']['StartedAt']) / 1000000000) if 'StartedAt' in info['State'] else "未知"
    ...
    def _list_images(self, client) -> str:
        ...
        if client:
            ...
            return self._run_docker_command("docker images --format 'table {{.ID}}\\t{{.Repository}}\\t{{.Tag}}\\t{{.Size}}'")
